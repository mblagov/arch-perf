...
public class PersonCUDStrategyStandaloneHotSection implements DataGenStrategy {

    private final int insertsCount;
    private final int updatesCount;
    private final int deletesCount;

    public PersonCUDStrategyStandaloneHotSection(int insertsCount, int updatesCount, int deletesCount) {
    ...
    public List<DataWithOperation> records() {
        ...
        List<Person> insertRecords = generator.generateRandomPeopleWithIdDob(this.insertsCount, LocalDate.now());
    ...
    private List<Person> refreshWithTheSameIds(List<Person> source, PersonGenerator generator) {
        return source.stream()
                .map(p -> generator.getRandomPersonDob(p.getId(), LocalDate.now()))
                .toList();
    }
}
