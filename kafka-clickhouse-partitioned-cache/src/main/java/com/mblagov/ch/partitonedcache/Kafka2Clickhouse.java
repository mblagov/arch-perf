package com.mblagov.ch.partitonedcache;

import com.clickhouse.jdbc.ClickHouseDataSource;
import com.google.gson.Gson;
import com.google.gson.GsonBuilder;
import com.mblagov.ch.partitonedcache.model.OplogMessage;
import com.mblagov.ch.partitonedcache.model.Person;
...
public class Kafka2Clickhouse {
    ...
    private static final Map<String, Map<String, Person>> caches = new HashMap<>();
    private static final Set<String> changedPartitions = new HashSet<>();
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            while (true) {
                ...
                consumerRecords.forEach(record -> {
                    ...
                    String partitionValue = String.valueOf(p.getDateOfBirth().getYear());
                    Map<String, Person> cache = caches.getOrDefault(partitionValue, new HashMap<>());
                    cache.put(p.getId(), p);
                    caches.put(partitionValue, cache);
                    changedPartitions.add(partitionValue);
                ...
                caches.forEach((key, value) -> {
                    if (changedPartitions.contains(key)) {
                        value.forEach((key1, pers) -> {
                            try {
